...
export default function Admin() {
  ...
  return <main className="admin-shell">
    <aside className="admin-side"><div className="admin-brand"><span>M</span><div><b>MELO CAFÉ</b><small>OPERACIONES</small></div></div><nav>{modules.map((item,index) => <button className={active === item ? "active" : ""} onClick={() => setActive(item)} key={item}><i>{icons[index]}</i>{item}</button>)}</nav><Link href="/">← Ver app del cliente</Link></aside>
